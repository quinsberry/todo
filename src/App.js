...
export default function App() {
  ...
  const onRemoveTask = (listId, taskId) => {
    axios
      ...
      .catch(() => {
        alert('Ups, we have some problems. Cannot delete the task.');
      })
  }
  const onEditTask = (listId, taskObj) => {
    const newTaskText = window.prompt('Edit your task', taskObj.text);
    if (!newTaskText) {
      return;
    }
    axios
  ...
  useEffect(() => {
    const listId = history.location.pathname.split('/')[2];
    if (lists) {
  ...
  return (
    <div className="todo">
      {mobileActive ? (
        <MobileSidebar activeItem={activeItem} history={history} lists={lists} setLists={setLists} onAddList={onAddList} colors={colors} toggleSidebar={toggleSidebar} isOpened={isOpened} />
      ) : (
          <>
